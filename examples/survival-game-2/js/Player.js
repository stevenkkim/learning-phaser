...
export default class Player extends MatterEntity {
  ...
  handlePlaceable(){
    if(!this.placeable){
      ...
      this.scene.input.on('pointerup', pointer =>{
        if(this.placeable.canPlace && this.inventory.selectedItemIsPlaceable){
          ...
          this.inventory.removeItem(this.inventory.selectedItem.name);
        } 
      });
  ...
  update(time,delta){
    this.placeables.forEach(p => p.update(time,delta));
    //this.placeables?.update(time,delta);

    if(this.dead) return;

    if(this.inventory.selectedItem){
      if(this.inventory.selectedItemIsPlaceable){
        ...
        this.placeable?.setVisible(false);
      }
